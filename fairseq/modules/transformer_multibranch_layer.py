# Copyright (c) Zekai Chen and its affiliates.
#
...
from fairseq.modules import LayerNorm, MultiheadAttention, LightweightConv, DynamicConv
...
class TransformerMbEncoderLayer(nn.Module):
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def __init__(self, args, index):
        ...
        self.self_attn_branches = self.get_all_branches(index, args)
        self.self_attn = self.build_self_attention(self.embed_dim, index, args)
    ...
    def get_all_branches(self, index, args):
...
class TransformerMbDecoderLayer(nn.Module):
    ...
    *args.decoder_normalize_before* to ``True``.
    ...
    def __init__(
        self, args, index, no_encoder_attn=False, add_bias_kv=False, add_zero_attn=False
    ):
        super().__init__()
    ...
    def get_all_branches(self, index, args, add_bias_kv=False, add_zero_attn=False):
    ...
    def build_self_attention(self, embed_dim, args, index, add_bias_kv=False, add_zero_attn=False):
        if args.decoder_branch_type is None:
            return MultiheadAttention(
                embed_dim,
    ...
    def forward(
        ...
        if self.onnx_trace and incremental_state is not None:
            ...
            if self_attn_padding_mask is not None:
                self_attn_state = [
                    [saved_state['prev_key'] for saved_state in saved_states],
                    [saved_state['prev_value'] for saved_state in saved_states],
                    [saved_state["prev_key_padding_mask"] for saved_state in saved_states],
                ]
            else:
                self_attn_state = [
                    [saved_state['prev_key'] for saved_state in saved_states],
    ...
    def reorder_incremental_state(
        ...
        for self_attn in self.self_attn_branches:
            self_attn.reorder_incremental_state(incremental_state, new_order)
